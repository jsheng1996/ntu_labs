...
int main(int argc, const char * argv[]){
	...
	scanf("%d", &i);
	printf("the values smaller than %d are:\n", i);
...
int smallestValue(BTNode *node) {
	int l, r;
    if(!node)
        return 1000; //assuming smallest number is not bigger than 1000
    /* an alternative solution will be to have a recursive function within the function to traverse the tree, then store each item in an array,
    and then iterate through the array to find the smallest value. this will solve the problem of having to assume the the max size of the min number*/
...
int ggc(BTNode *node){
    if(!node)
        return 0;
    if(node->left||node->right)
        return 1+ggc(node->left)+ggc(node->right);
    else
        return 0;
}
